...
# Files to process: 4-6 laptop files at a time
BATCH_1_FILES = [
    "laptops2.json",
    "macbooks.json",
    "2-in-1-laptops.json",
...
def main():
    print("=" * 80)
    print("CLOUDINARY BULK IMAGE UPLOADER (BATCH 1: LAPTOPS)")
    print("=" * 80)
    ...
    for filename in BATCH_1_FILES:
        if process_file_optimized(filename, NEWCOMPUTING_DIR):
            processed_count += 1
        # Delay between files
        time.sleep(2)
    ...
    print("\n" + "=" * 80)
    print(f"✅ BATCH 1 COMPLETED: {processed_count}/{len(BATCH_1_FILES)} files processed")
