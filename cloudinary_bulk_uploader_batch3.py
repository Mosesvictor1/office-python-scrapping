...
# Directories
HPOUTPUT_DIR = Path("/Users/newupdate/Campushut/untitled folder/ScrappingProduct/office-python-scrapping/HP2 WEBSITE/Hpoutput")
NEWCOMPUTING_DIR = Path("/Users/newupdate/Campushut/untitled folder/ScrappingProduct/office-python-scrapping/NewComputing")

# Files to process: Batch 3 (Additional categories)
BATCH_3_FILES = [
    ("desktops.json", HPOUTPUT_DIR),
    ("workstations.json", HPOUTPUT_DIR),
    ("monitors.json", HPOUTPUT_DIR),
    ("inks_toner.json", HPOUTPUT_DIR),
    ("computer monitors.json", NEWCOMPUTING_DIR),
    ("Desktop PCs.json", NEWCOMPUTING_DIR),
]
...
def main():
    print("=" * 80)
    print("CLOUDINARY BULK IMAGE UPLOADER (BATCH 3: DESKTOPS, MONITORS, INK/TONER)")
    print("=" * 80)
    print(f"Cloud: {CLOUDINARY_CLOUD_NAME}")
    print(f"Upload Preset: {CLOUDINARY_UPLOAD_PRESET}")
    print(f"Files to process: {len(BATCH_3_FILES)}")
    print("=" * 80)
    
    start_time = time.time()
    processed_count = 0
    
    for filename, directory in BATCH_3_FILES:
    ...
    print("\n" + "=" * 80)
    print(f"✅ BATCH 3 COMPLETED: {processed_count}/{len(BATCH_3_FILES)} files processed")
