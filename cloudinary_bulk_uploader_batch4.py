...
# Files to process: Batch 4 (Remaining NewComputing files)
BATCH_4_FILES = [
    ("Data storage.json", NEWCOMPUTING_DIR),
    ("Laptop bags and cases.json", NEWCOMPUTING_DIR),
    ("Printers, scanners and ink.json", NEWCOMPUTING_DIR),
    ("componenets and upgrade.json", NEWCOMPUTING_DIR),
    ("computer accessories.json", NEWCOMPUTING_DIR),
    ("iPad, Tablets & eReaders.json", NEWCOMPUTING_DIR),
]
...
def main():
    print("=" * 80)
    print("CLOUDINARY BULK IMAGE UPLOADER (BATCH 4: NEWCOMPUTING REMAINING)")
    print("=" * 80)
    print(f"Cloud: {CLOUDINARY_CLOUD_NAME}")
    print(f"Upload Preset: {CLOUDINARY_UPLOAD_PRESET}")
    print(f"Files to process: {len(BATCH_4_FILES)}")
    print("=" * 80)
    
    start_time = time.time()
    processed_count = 0
    
    for filename, directory in BATCH_4_FILES:
    ...
    print("\n" + "=" * 80)
    print(f"✅ BATCH 4 COMPLETED: {processed_count}/{len(BATCH_4_FILES)} files processed")
